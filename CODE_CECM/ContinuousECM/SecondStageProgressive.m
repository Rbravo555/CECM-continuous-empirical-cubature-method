function  [xOLD,wOLD,DATALOC,POLYINFO,VARC,HISTORY] =...
    SecondStageProgressive(xOLD,wOLD,b,DATALOC,VAR_SMOOTH_FE,VARC,HISTORY,POLYINFO)
...
if nargin == 0
    load('tmp_216.mat')
    DATALOC.VARIABLE_ITERATIONS_SECOND_STAGE = 0 ; 
...
while CONVERGENCE ==1  %&& length(xOLD)>1
    ...
    [xNEW,wNEW,CONVERGENCE,DATALOC,POLYINFO,VARC ] = Eliminate1PointPROG(xOLD,wOLD,b,DATALOC,VAR_SMOOTH_FE,VARC,POLYINFO) ;
    if CONVERGENCE == 1
        
        [HISTORY,INDnneg,CurrentITERglo] = HistoryPointsUpdate(wNEW,xNEW,HISTORY,DATALOC,iter,CurrentITERglo) ;
