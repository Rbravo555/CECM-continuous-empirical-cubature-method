...
while  nF>=TOL && k<= kMAX && nITER_increase<=maxITER_increase
    ...
    VARCnew.NumberOfIterationsTotal = VARCnew.NumberOfIterationsTotal +1 ; 
    if ISOUT == 1
        break
    end
...
% ------------------------------------------------------
%if ( ISOUT ==1  ) || (k>kMAX  &&  nF>DATALOC.TOL_low ) || nITER_increase
%>maxITER_increase  % COrrection made on July 13th 2023


if ( ISOUT ==1  ) || (k>kMAX   ) || nITER_increase >maxITER_increase
    %   iremove = iremove + 1;
    %   xNEW = xOLD ;
    %   wNEW = wOLD ;
